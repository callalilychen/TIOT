/*
 * This file is adapted from the RIOT project
 *
...
static  uint8_t TC8_CHACHA8_BLOCK0[64] = {
...
static  uint8_t TC8_CHACHA8_BLOCK1[64] = {
...
static  uint8_t TC8_CHACHA12_BLOCK0[64] = {
...
static  uint8_t TC8_CHACHA12_BLOCK1[64] = {
...
static  uint8_t TC8_CHACHA20_BLOCK0[64] = {
...
static  uint8_t TC8_CHACHA20_BLOCK1[64] = {
...
//#define ORIGIN_TEST
static inline void _test_crypto_chacha(unsigned rounds, unsigned keylen,
                                ...
                                const uint8_t block0[64], const uint8_t block1[64])
{
    ...
    if(0!= chacha_init(&ctx, rounds, key, keylen, iv)){
      DEBUG("FAIL, chacha init\n");
    }
#ifdef ORIGIN_TEST
    if(0 != memcmp(ctx.state, after_init, 64)){
      DEBUG("FAIL, chacha state\n");
    }
#endif
#ifdef SIGNAL
  signalHigh();
...
#endif
#ifdef ORIGIN_TEST
    if(0!=memcmp(block, block0, 64)){
      DEBUG("FAIL, chacha computation\n");
    }else{
      DEBUG("OK\n");
    }
#else
    memcpy(block0, block, 64);
#endif
...
#endif
#ifdef ORIGIN_TEST
    if(0!=memcmp(block, block1, 64)){
      DEBUG("FAIL, chacha computation\n");
    }else{
      DEBUG("OK\n");
    }
#else
    memcpy(block1, block, 64);
#endif
}

static inline void test_crypto_chacha8_tc8(void)
{
    _test_crypto_chacha(8,  16, TC8_KEY, TC8_IV, TC8_AFTER_INIT,
                        TC8_CHACHA8_BLOCK0, TC8_CHACHA8_BLOCK1);
}

static inline void test_crypto_chacha12_tc8(void)
{
    _test_crypto_chacha(12, 16, TC8_KEY, TC8_IV, TC8_AFTER_INIT,
                        TC8_CHACHA12_BLOCK0, TC8_CHACHA12_BLOCK1);
}

static inline void test_crypto_chacha20_tc8(void)
{
...
inline static void printChaCha20(void){
  int i;
  DEBUG("ChaCha8 IV\n\r");
  for (i=0; i< 8; i++){
    DEBUG("%x", TC8_IV[i]);
  ...
  DEBUG("\n\r");
  DEBUG("ChaCha8 Value Block 0\n\r");
  for (i=0; i< 64; i++){
    DEBUG("%x", TC8_CHACHA8_BLOCK0[i]);
  }
  DEBUG("\n\r");
  DEBUG("ChaCha8 Value Block 1\n\r");
  for (i=0; i< 64; i++){
    DEBUG("%x", TC8_CHACHA8_BLOCK1[i]);
  }
  DEBUG("\n\r");
  DEBUG("ChaCha12 Value Block 0\n\r");
  for (i=0; i< 64; i++){
    DEBUG("%x", TC8_CHACHA12_BLOCK0[i]);
  }
  DEBUG("\n\r");
  DEBUG("ChaCha12 Value Block 1\n\r");
  for (i=0; i< 64; i++){
    DEBUG("%x", TC8_CHACHA12_BLOCK1[i]);
  }
  DEBUG("\n\r");
  DEBUG("ChaCha20 Value Block 0\n\r");
  for (i=0; i< 64; i++){
    DEBUG("%x", TC8_CHACHA20_BLOCK0[i]);
  }
  DEBUG("\n\r");
  DEBUG("ChaCha20 Value Block 1\n\r");
  for (i=0; i< 64; i++){
    DEBUG("%x", TC8_CHACHA20_BLOCK1[i]);
  }
  DEBUG("\n\r");
}
...
int testChaCha20(int newValue){
#ifndef ORIGIN_TEST
  for(int i = 0; i < 8;){
    TC8_IV[i++] = (uint8_t)(newValue>>8);
    TC8_IV[i++] = (uint8_t)newValue;
  }
#endif
#ifdef SIGNAL
  signalHigh();
  ...
  signalLow();
#endif
  test_crypto_chacha8_tc8();
  test_crypto_chacha12_tc8();
  test_crypto_chacha20_tc8();
#ifndef ORIGIN_TEST
  printChaCha20();
#endif
  return 1;
