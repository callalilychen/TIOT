...
static void ports_init(void)
{
  ...
  P4OUT = 0;    /* Port4 Output: 00000000 = 0x00 */
     ...
     *  TODO P5.0 controls TMP102 power (active high) -> output, GPIO, default to GND
     *  TODO P5.1 is I2C's SDA (data) pin -> input (by default/changeable), special function, default to GND
     *  P5.2 is XT2IN: Q4 CSTCR4M00G15L99
     *  P5.3 is XT2OUT
     *  P5.4 is XIN: LFXTCLK
     *  P5.5 is XOUT
     *  TODO P5.6 controls one of Z1's LEDs (active low) -> output, GPIO, default to Vcc
     *  TODO P5.7 is the Flash chip (M25P16) active-low HOLD line -> output, GPIO, default to Vcc
     * NOTES :
...
#ifdef SOURCE_XT2
  P5SEL |= BIT2 | BIT3;    /* Port5 Select: 00111100 = 0x00 */
  P5DIR &= ~BIT2;    /* Port5 Direction: 00101000 = 0xF5 */
#endif
#ifdef SOURCE_LFXT
  P5SEL |= BIT4 | BIT5;    /* Port5 Select: 00111100 = 0x00 */
  P5DIR &= ~BIT5;    /* Port5 Direction: 00101000 = 0xF5 */
#endif
    /* Port 6:
...
{
  // TODO calibrate
#ifdef  CALIBRATE_MSP430_DCO_NOTDEFINED
#define DELTA    (F_CPU / (F_RC_OSCILLATOR / 8))
    ...
    UCSCTL0 = DCO0 + DCO1 + DCO2 + DCO3 + DCO4; // DCO frequency range in 28.2MHZ 
    UCSCTL1 = DCORSEL_4;  /* Select DCO frequency range 3.2 ~ 12.3 MHz*/
    //UCSCTL2 = FLLD_0 | FLLN0;         /* FIXME Set FLLD = 2 and FLLN =1*/
    UCSCTL2 = FLLD__16 | 0x1; // D = 16, N = 1
     /* UCSCTL3 = SELREF__XT2CLK;Select FLL reference clock source by XT2CLK, which is 4MHz; FLLREFDIV = n = 0*/
    UCSCTL3 = SELREF__XT2CLK| FLLREFDIV__8; // n = 8 for XT2CLK, FLLREFCLK clock source; DCOCLK = D * (N +1) * (FLLREFCLK / n); DCOCLKDIV = (N +1) * (FLLREFCLK / n);
    //UCSCTL4 = SELS__XT2CLK | SELM__XT2CLK;        /* FIXME get MCLK and SMCLK from DCO, without divisor */
    UCSCTL4 =SELS__DCOCLK | SELM__DCOCLK;        /* FIXME get MCLK and SMCLK from DCO, without divisor */
    //UCSCTL5 = DIVA_5 + DIVS_1; // ACLK from the DCOCLKDIV 32 frequency to be SMCLK frequency from 2 DCOCLK to get
    UCSCTL5 = DIVS_1;
  // Final MCLK: 16MHz SMCLK: 8MHz, TODO ACLK: 32kHz 
    __bic_SR_register(SCG0); // Enable the FLL control loop
#endif
}


/* "public" specific initialization function for the MSP430F5529 hardware */
